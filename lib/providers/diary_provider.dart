...
class DiaryProvider extends ChangeNotifier {
  ...
  Future<void> init() async {
    ...
    if (SupabaseConfig.isConfigured) {
      _entries = await SupabaseService.fetchEntries();
    } else {
  ...
  Future<DiaryEntry> createAndTransformEntry({
    ...
    try {
      ...
      if (SupabaseConfig.isConfigured) {
        // Run in background so UI isn't blocked by network
        SupabaseService.insertEntry(entry);
      } else {
  ...
  Future<void> toggleFavorite(String entryId) async {
    ...
    if (SupabaseConfig.isConfigured) {
      SupabaseService.updateEntry(updatedEntry);
    } else {
      await StorageService.saveEntries(_entries);
    }
    notifyListeners();
  ...
  Future<void> deleteEntry(String entryId) async {
    ...
    if (SupabaseConfig.isConfigured) {
      SupabaseService.deleteEntry(entryId);
    } else {
      await StorageService.saveEntries(_entries);
    }
    notifyListeners();
